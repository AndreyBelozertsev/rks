...
class CRest
{
	...
	//const C_REST_WEB_HOOK_URL = 'https://' . env('B24_PORTAL') .'.bitrix24.ru/rest/' . env('B24_USER_ID') . '/' . env('B24_TOKEN') . '/';


	protected static string $c_rest_web_hook_url;
	/**
	...
	public static function setLog($arData, $type = '')
	{
		...
		if(!defined("C_REST_BLOCK_LOG") || C_REST_BLOCK_LOG !== true)
		{
			...
			{
				$path = storage_path() . '/logs/bitrix24/';
			}
			$path .= date("Y-m-d/H") . '/';
	...
	{
		return 'https://' . env('B24_PORTAL') .'.bitrix24.ru/rest/' . env('B24_USER_ID') . '/' . env('B24_TOKEN') . '/';
	}
}